/**
 * ETL runner for IGJ company registry data pipeline.
 * Run with: npx tsx scripts/run-etl-cnv.ts
 *
...
import { fetchIgjData } from '../src/etl/cnv-securities'
import { transformIgjAll } from '../src/etl/cnv-securities'
import { loadIgjAll } from '../src/etl/cnv-securities'
...
async function main(): Promise<void> {
  ...
  // -- 3. Fetch IGJ data --
  console.log('Fetching IGJ company registry data...')
  ...
  console.log(`  Board members:     ${transformed.boardMembers.length}`)
  console.log(`  BOARD_MEMBER_OF:   ${transformed.boardMemberOfRels.length}`)
  ...
  if (transformed.maybeSameAsRels.length > 0) {
    console.log('\n  Matched politicians <-> board members:')
    for (const m of transformed.maybeSameAsRels.slice(0, 20)) {
      const member = transformed.boardMembers.find((b) => b.igj_authority_id === m.authority_id)
      console.log(`    ${m.politician_id} <-> ${member?.name ?? m.authority_id} (confidence: ${m.confidence})`)
    }
