/**
 * ETL runner for Compr.ar ordenes de compra pipeline.
 * Run with: npx tsx scripts/run-etl-comprar.ts
 *
...
import { fetchComprarData } from '../src/etl/comprar'
import { transformComprarAll } from '../src/etl/comprar'
import { loadComprarAll } from '../src/etl/comprar'
...
async function main(): Promise<void> {
  ...
  // -- 3. Fetch Compr.ar data -------------------------------------------------
  console.log('Fetching Compr.ar ordenes de compra...')
  const fetchStart = Date.now()
  const fetched = await fetchComprarData()
  console.log(`  Fetch complete in ${formatDuration(Date.now() - fetchStart)}`)
  console.log(`  Orders:  ${fetched.stats.totalOrders} (years: ${fetched.stats.orderYears.join(', ')})\n`)

  // -- 4. Transform -----------------------------------------------------------
  console.log('Transforming data...')
  const transformStart = Date.now()
  const transformed = transformComprarAll({
  ...
  const loadResult = await loadComprarAll(transformed)
  ...
  console.log(`Compr.ar ETL pipeline ${loadResult.totalErrors > 0 ? 'completed with errors' : 'completed successfully'}`)
  ...
  if (transformed.maybeSameAsRels.length > 0) {
    console.log('\n  Matched politicians <-> contractors:')
    for (const m of transformed.maybeSameAsRels.slice(0, 20)) {
      const contractor = transformed.contractors.find((c) => c.contractor_id === m.contractor_id)
      console.log(`    ${m.politician_id} <-> ${contractor?.name ?? m.contractor_id} [confidence: ${m.confidence}]`)
    }
...
main().catch((error) => {
  console.error('Compr.ar ETL pipeline failed:', error)
