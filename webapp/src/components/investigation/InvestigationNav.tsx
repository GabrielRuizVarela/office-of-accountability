 ...
 * Config-driven tab configuration via investigation registry.
...
// ---------------------------------------------------------------------------
// Tab metadata — maps each TabId to its route suffix and bilingual label
// ---------------------------------------------------------------------------

const TAB_META: Readonly<Record<TabId, { href: string; label: Record<Lang, string> }>> = {
  ...
  grafo: { href: '/grafo', label: { en: 'Graph', es: 'Grafo' } },
  dinero: { href: '/dinero', label: { en: 'Money', es: 'Dinero' } },
...
const HOME_TAB: Readonly<{ href: string; label: Record<Lang, string> }> = {
  href: '',
...
export function InvestigationNav({ slug }: InvestigationNavProps) {
  ...
  // Look up config by slug, then try caso-prefixed slug for legacy routes
  const config = getClientConfig(slug) ?? getClientConfig(`caso-${slug}`)
  const tabIds = config?.tabs ?? DEFAULT_TAB_IDS
  const tabDefs = [HOME_TAB, ...tabIds.map((id) => TAB_META[id])]
