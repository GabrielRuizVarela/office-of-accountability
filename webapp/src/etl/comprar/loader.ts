/**
 * ETL Loader -- batch MERGE of Compr.ar data into Neo4j.
 *
...
import type { ComprarTransformResult, MaybeSameAsContractorRelParams } from './transformer'
...
async function loadMaybeSameAsRels(
  items: readonly MaybeSameAsContractorRelParams[],
): Promise<LoadStepResult> {
...
export interface ComprarLoadResult {
  readonly steps: readonly LoadStepResult[]
  readonly totalErrors: number
  readonly durationMs: number
}

export async function loadComprarAll(data: ComprarTransformResult): Promise<ComprarLoadResult> {
