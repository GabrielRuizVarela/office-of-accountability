...
11. **Motor de investigación autónomo** — automated pipeline (ingest → verify → enrich → analyze → report) with human review gates at each step. LLM abstraction layer (llamacpp/Qwen, OpenAI, Anthropic, Ollama) with three execution modes (single, tool-agent, swarm). Source connectors (REST API, file upload, custom script) with two-pass dedup. Graph algorithms (centrality, community detection, anomaly, temporal) produce hypothesis proposals. All config in Neo4j (SourceConnector, PipelineConfig, PipelineStage, Gate, Proposal, AuditEntry nodes). Engine outputs map to platform concepts: proposed nodes → Bronze-tier, verified promotions → Silver-tier, LLM hypotheses → Claim nodes, reports → Investigation documents.
12. **Investigation standardization** — three investigations (Caso Libra, Caso Finanzas Politicas, Caso Epstein) unified under `InvestigationConfig` schema subgraph in Neo4j. Generic labels (`Person`, `Organization`, `Event`, etc.) with `caso_slug` property for namespace isolation, prefixed IDs (`{caso_slug}:{local_id}`). Unified API at `/api/casos/[casoSlug]/*` (7 endpoints: graph, nodes/[type], node/[slug], timeline, schema, submissions, stats). Schema-aware query builder generates Cypher dynamically from `NodeTypeDefinition` nodes. Config-driven frontend with `InvestigationClientConfig` registry (tabs, feature flags, hero text, chapters). Per-investigation backend modules (`lib/caso-{slug}/`) with types, queries, transform, config. Caso Libra migrates from `CasoLibra*` prefixed labels. Finanzas Politicas imports narrative data from static TypeScript. Epstein imports full rhowardstone dataset (606 KG entities + 1,614 persons registry) with victim pseudonymization.
...
| 10 | Motor de Investigación Autónomo | Automated pipeline (ingest→verify→enrich→analyze→report) with human gates, LLM abstraction, source connectors, graph algorithms | M9 |
...
**Given** an investigation with configured source connectors and pipeline,
**When** `POST /api/engine/[id]/run` is called,
**Then** the ingest stage runs connectors, creates bronze nodes with dedup, and sets `PipelineState.status` to `gate_pending`.

**Given** a gate-pending pipeline,
**When** proposals are batch-approved via the API,
**Then** approved proposals are applied to the graph, an AuditEntry with SHA-256 hash chain is created, a Snapshot is auto-saved, and the pipeline advances to the next stage.
